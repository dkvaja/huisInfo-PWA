...
import { Avatar, Container, Grid, Typography, Divider, Card, CardHeader, List, ListItem, ListItemText, ListItemAvatar, Badge, IconButton, CardMedia, Dialog, DialogContent, DialogTitle, Box, ListItemIcon } from "@material-ui/core";
import { withStyles } from '@material-ui/core/styles';
import { Schedule, Chat, Assignment, Close, KeyboardArrowLeft, KeyboardArrowRight, AttachFile, InfoOutlined, TimerOff } from "@material-ui/icons"
import { getDateText, md2plaintext, formatDate, formatTime, authHeader, history } from "../../_helpers"
...
class Page extends React.Component {
    ...
    componentDidMount() {
        ...
        if (selected) {
            ...
            this.UpdateMessages();
            this.UpdateActions();
            this.redirectToDossiers();
        }
        if (this.props.rights && this.props.rights['page.canShowHomePage']) {
    ...
    redirectToDossiers() {
        ...
        if (selected) {
            if (this.props.rights && !this.props.rights['page.canShowHomePage'])
                history.replace(`/werk/${selected.projectNo}/dossier`);
        }
    }
    ...
    renderMessages() {
        ...
        const baseUrl = '/werk/' + (selected && selected.projectNo);

        return (
            <Card className={classes.card}>
                <Link to={baseUrl + '/berichten'}>
                    <CardHeader title={
...
function mapStateToProps(state) {
    ...
    const { selected, all, rights } = buildings;
...
export { connectedPage as ProjectDashboard };